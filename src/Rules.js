...
// evaluate value of each dice number
class TotalOneNumber extends Rule {
  // multiply value of dice by count of dice
  evalRoll = dice => {
    return this.val * this.count(dice, this.val);
  };
}

// sum all distribution of dice
class SumDist extends Rule {
  evalRoll = dice => {
    // do any of the counts meet of exceed this distribution?
...
const threeOfKind = new SumDist({
  count: 3,
  description: "Sum all dice if 3 are the same"
});
const fourOfKind = new SumDist({
  count: 4,
...
const chance = new SumDist({ count: 0, description: "Sum of all dice" });
